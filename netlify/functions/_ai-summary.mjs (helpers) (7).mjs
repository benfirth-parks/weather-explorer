...
TASK: Produce a short 2–4 sentence summary of the previous 24 hours from the provided station data. Focus on elevation and regional differences. Identify outliers. Give ranges rather than single values.
...
- Use standard forecaster shorthand: "alpine", "treeline", "below treeline"; compass bearings ("SW", "WNW"); "gusts", "peak gusts", "new snow", "HS", "freezing level" only when supported by data. Wind verbs: "strengthened", "eased", "backed", "veered". Temperature verbs: "warmed", "cooled", "inversion", "isothermal".
...
WHAT TO INCLUDE:
- Cover elevation contrast (alpine vs treeline vs below treeline) whenever the data shows a meaningful difference.
- Cover regional contrast (north / south / east / west of Lake Louise, or combinations) whenever it shows a meaningful difference. Each station's \`ns\` and \`ew\` fields give its region.
- Give values as RANGES across the stations in the referenced band/region: "alpine highs 4–9 °C", "gusts 40–65 km/h", "1–4 mm". Only use a single value when one station is the sole datapoint OR when all stations report the same value.
- Include at least one wind observation with strength + direction.
- Include at least one temperature reference.
- Include precipitation if any station recorded any.
- Call out ONE clear outlier if there is one — a station that broke well outside the band/region range for temperature, wind, or precipitation. Station names ARE permitted when naming an outlier: use the \`name\` field verbatim (this is the same label the dashboard's 24h station summary table shows, including any trailing operator suffix like " - AB Env", "- Fire", or "- AB ENV/ LLSA"). Do not shorten, tidy, or re-punctuate the name. Don't force an outlier if none stands out.

WHAT TO AVOID:
- Don't name stations for routine readings that fit inside the band/region range.
- Don't mention operational groups or agencies as prose (no "Visitor Safety", "VS", "Fire crew", "Banff Fire"). Suffixes that appear inside a station's verbatim \`name\` (e.g. "- Fire", "- AB Env") are fine when quoting that name for an outlier.
- Don't mention Jasper — the snapshot excludes those stations.
- Don't invent numbers. Only use values present in the JSON.
- Don't add headers, bullets, or preamble. Return only the summary prose.`;
