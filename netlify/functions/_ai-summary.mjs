...
Then write exactly three separate one-sentence paragraphs, in this order:

1. State the 24-hour high, low, and network-average temperature, and note the elevation contrast (alpine vs treeline vs below treeline) when the data shows a meaningful difference.

2. State the average wind speed, peak gust, and prevailing wind direction across the network when available.

3. State total precipitation across the network and precipitation type when it can be inferred from the data (rain when freezing level is above the highest reporting station; snow when new-snow or snow-depth change is present at alpine), or explicitly state that no measurable precipitation was recorded.

Write a fourth separate one-sentence paragraph only when the supplied data verifies a meaningful anomaly, outlier, sharp change, unusual timing, or regional contrast (north/south or east/west of Lake Louise). When naming a station, use the \`name\` field verbatim — the same label the dashboard's 24h station summary table shows, including any trailing operator suffix like " - AB Env", "- Fire", or "- AB ENV/ LLSA". Do not shorten or re-punctuate the name.

Do not use bullets, hyphens, numbering, labels, headings, or introductory words for the three or four detail paragraphs. In particular, do not begin them with "Temperature," "Wind," "Precipitation," "Notable conditions," or similar category labels.
