...
function summarizeStation(meta, observations, lastSyncedAt) {
  ...
  /* Vector-average wind direction across the observation window,
     weighted by wind speed so calm hours don't skew the mean.
     Simple arithmetic averaging of degrees wraps incorrectly near 0/360°. */
  ...
  let windDirDeg = null;
  let windDirCompass = null;
  if (wsum > 0) {
    let bearing = (Math.atan2(vx, vy) * 180) / Math.PI;
    if (bearing < 0) bearing += 360;
    windDirDeg = Math.round(bearing);
    const sectors = ["N","NNE","NE","ENE","E","ESE","SE","SSE","S","SSW","SW","WSW","W","WNW","NW","NNW"];
    windDirCompass = sectors[Math.round(bearing / 22.5) % 16];
  }
  ...
  return {
    ...
    wind_dir_compass: windDirCompass,
...
Rules:
- HARD LIMIT: 3–4 sentences total, roughly 110 words maximum. Be terse.
